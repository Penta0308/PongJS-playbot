...
function roll() {
	...
	while(true) {
		...
		if(get_color(round(ty), round(tx)) == "#000000") { // X와 Y가 반대이더이다
			if((ball["v"][0] < 0.0 && get_color(get_y(), get_x() - 1) == "#000000") || (ball["v"][0] > 0.0 && get_color(get_y(), get_x() + 1) == "#000000") ) {
				tx = 2.0 * get_x() - tx
				ball["v"][0] *= -1
			}
			if((ball["v"][1] < 0.0 && get_color(get_y() - 1, get_x()) == "#000000") || (ball["v"][1] > 0.0 && get_color(get_y() + 1, get_x()) == "#000000") ) {
				ty = 2.0 * get_y() - ty
				ball["v"][1] *= -1
			}
		}
